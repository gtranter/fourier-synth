...
public class FourierRefactored extends Applet {

  // Max. |ak|,|bk|
  static final int CONTROL_RANGE = 50;

  // "k_max"

  static final int HARMONICS = 6;

  // a_k = a[k]/INT_MULT

  static final double INT_MULT = 10.0;

  // time base

  static final double TIM_MULT = 40.0/Math.PI;

  // Audio??

  static final boolean AUDIO = true;

  private int a [] = new int[HARMONICS+1];

  private int b [] = new int[HARMONICS+1];

  private Label la_a [] = new Label[HARMONICS+1];

  private Label la_b [] = new Label[HARMONICS+1];

  private Scrollbar s_a [] = new Scrollbar[HARMONICS+1];
  private Scrollbar s_b [] = new Scrollbar[HARMONICS+1];
  ...
  static final void gbcon(GridBagLayout gridbag, GridBagConstraints c,
  ...
  public void init() {
...

    {
      ...
      s_a[i] = new Scrollbar(Scrollbar.HORIZONTAL,a[i],1,-CONTROL_RANGE,CONTROL_RANGE);
...

      {
        ...
        s_b[i] = new Scrollbar(Scrollbar.HORIZONTAL,b[i],1,-CONTROL_RANGE,CONTROL_RANGE);
  ...
  private final void plot() {
    ...
    // One period, TIM_MULT dependent! This needs clean up...
	 ...
	 int graphWidth = 400;
	 int waveCount = 5;
    int wavelength = graphWidth / waveCount;
...

    {
...

      {
        ...
        y += a[harmonic]/INT_MULT*Math.cos(harmonic*x/TIM_MULT);

        y += b[harmonic]/INT_MULT*Math.sin(harmonic*x/TIM_MULT);
...

      {
        ...
        for (int j = 0; j < 400; j += wavelength)

        dbg.drawLine(x-1+j, ya, x+j, iy);
    ...
    for (int j = wavelength; j < 400; j+=wavelength)
